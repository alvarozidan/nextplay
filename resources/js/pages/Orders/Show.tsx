...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string; icon: string; desc: string }> = {
    pending:    { label: 'Menunggu Pembayaran', color: 'text-amber-700',   bg: 'bg-amber-50 border-amber-200',   icon: '⏳', desc: 'Transaksi menunggu konfirmasi pembayaran.' },
    paid:       { label: 'Pembayaran Diterima', color: 'text-blue-700',    bg: 'bg-blue-50 border-blue-200',     icon: '💳', desc: 'Pembayaran berhasil diterima, sedang diproses.' },
    processing: { label: 'Sedang Diproses',     color: 'text-purple-700',  bg: 'bg-purple-50 border-purple-200', icon: '⚙️', desc: 'Item sedang dikirimkan ke akun game kamu.' },
    completed:  { label: 'Transaksi Selesai',   color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200', icon: '✅', desc: 'Item berhasil masuk ke akun game kamu.' },
    failed:     { label: 'Transaksi Gagal',     color: 'text-red-700',     bg: 'bg-red-50 border-red-200',       icon: '❌', desc: 'Transaksi gagal. Hubungi support jika kamu sudah membayar.' },
};
...
export default function OrderShow({ order }: { order: Order }) {
    ...
    const status = STATUS_MAP[order.status] ?? {
        label: order.status, color: 'text-slate-700', bg: 'bg-slate-50 border-slate-200', icon: '•', desc: '',
    };
